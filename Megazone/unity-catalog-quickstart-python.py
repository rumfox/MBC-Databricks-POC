...
spark.sql("USE mbc_poc_unity_catalog.mbc_poc_schema")
...
spark.sql("CREATE OR REPLACE TEMP VIEW content_owner_demographics_a1_temp_view (date DATE,	channel_id STRING,	video_id STRING,	claimed_status STRING,	uploader_type STRING,	live_or_on_demand STRING,	subscribed_status STRING,	country_code STRING,	age_group STRING,	gender STRING,	views_percentage DOUBLE,	video_title STRING,	channel_title STRING,	program_id STRING,	program_title STRING,	matching_method STRING,	total_view_count INT,	total_comment_count INT,	total_like_count INT,	total_dislike_count INT,	video_publish_datetime TIMESTAMP) USING csv OPTIONS ('path' 's3://mbcinfo/upload/content_owner_demographics_a1/', 'header' 'true')")
...
spark.sql("CREATE OR REPLACE TABLE content_owner_demographics_a1 USING DELTA PARTITIONED BY (month) COMMENT 'content_owner_demographics_a1' AS SELECT *, month(date) as month FROM content_owner_demographics_a1_temp_view")

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from content_owner_demographics_a1 limit 10;

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE EXTENDED content_owner_demographics_a1;

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT DISTINCT month
